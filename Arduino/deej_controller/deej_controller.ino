...
void setup() {
  ...
  if(useEEPROM==noEEPROM){
  }
  else if(useEEPROM==extEEPROM){
...
void UpdateLCD() {
    ...
    else if (state == valueScreen) { //put slider on screen
      lcd.setCursor(0, 0);
      lcd.print(sliderNames[sliderNumber]);
      lcd.setCursor(15,0);
      lcd.write(sliderToIndicator(displayValue[sliderNumber]));
...
void RotateRight(){
  if (state == menuScreen) { //scrolling between all the slides
    sliderNumber++;
    if (sliderNumber > (amountSliders - 1)) {
      sliderNumber = 0;
    }
