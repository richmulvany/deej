...
func SignificantlyDifferent(old float32, new float32, noiseReductionLevel string) bool {
	...
	case noiseReductionLow:
		significantDifferenceThreshold = 0.001//default value would be 0.015 but changed it to 0.001 so the noiseReduction (almost) doesn't influence the slider value anymore
		break
	default:
